// cap cd "C:\Users\h1mad06res\Dropbox (STL Fed - Research)\Supply Vs. Demand Shocks\Composition Effects\Occupation_Data"
...
quietly infix                ///
  ...
  int     ind1950   82-84    ///
  double  earnwt    85-94    ///
  double  compwt    95-108   ///
  double  hourwage  109-113  ///
  double  earnweek  114-121  ///
  using `"cps_00007.dat"'
...
save cps_monthly.dta, replace


// Plot figure
set more off

use cps_monthly.dta, clear

// Adjust Food & Drink
replace ind = 8680 if ind == 8690

// Keep only relevant occupation by industry
keep if (ind == 5170 & occ == 5240) | (ind == 8660 & occ == 4000) | (ind == 8680 & occ == 4000)

// Drop missing hourly wage info
drop if hourwage == 999.99

// Collapse
gen hourwage_wgt = hourwage*earnwt

collapse (sum) hourwage_wgt earnwt, by (year month ind occ)

gen hourwage = hourwage_wgt/earnwt

// Generate panel and date variables
gen date = ym(year,month)
format date %tm

egen occind = group(occ ind)

xtset occind date, m

// Plot
twoway (line hourwage date if occind==1, lwidth(thick)) (line hourwage date if occind==2, lpattern(longdash) lwidth(thick)) ///
	(line hourwage date if occind==3, lpattern(shortdash) lwidth(thick)), ///
	legend(label(1 "Chefs, Accommodation") label(2 "Chefs, Food & Drink") label(3 "Customer Service, Clothing Stores")) ///
	 xline(722, lcol(black) lpattern(dash)) xline(723, lcol(black) lpattern(dash)) ///
	 ytitle("Hourly Wage") graphregion(color(white))
	 graph export ../figures/wages2020_2021.pdf, replace
